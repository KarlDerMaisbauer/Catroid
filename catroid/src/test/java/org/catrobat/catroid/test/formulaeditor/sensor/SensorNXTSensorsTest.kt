...
class SensorNXTSensorsTest (
    ...
    fun setup() {
        ...
        PowerMockito.`when`(ServiceProvider.getService(CatroidService.BLUETOOTH_DEVICE_SERVICE)).thenReturn(mockBlueToothService)
        if (returnNXT == NXT) {
        ...
        PowerMockito.`when`(mockBlueToothService.getDevice(BluetoothDevice.LEGO_NXT)).thenReturn(mockNXT)
    }
    ...
    companion object {
        ...
        private var mockBlueToothService: BluetoothDeviceService = mock(BluetoothDeviceService::class.java)
