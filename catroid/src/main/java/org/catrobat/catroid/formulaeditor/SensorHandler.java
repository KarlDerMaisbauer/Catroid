...
public final class SensorHandler implements SensorEventListener, LocationListener,
	...
	static public final float[] rotationMatrix = new float[16];
	static public final float[] rotationVector = new float[3];
	static public float[] accelerationXYZ = new float[3];
	public static  float signAccelerationZ;
	private final float[] gravity = {0f, 0f, 0f};
	private boolean useLinearAccelerationFallback;
	static public boolean useRotationVectorFallback;
	...
	public static double getXAccordingToRotation(double x, double y) {
		int rotate = rotateOrientation();
		return rotate != 0 ? -y * rotate : x;
	}

	public static double getYAccordingToRotation(double x, double y) {
		int rotate = rotateOrientation();
